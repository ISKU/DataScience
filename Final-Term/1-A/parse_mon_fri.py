...
for rent in tashu_dict :
    ...
    if getDay[datetime.date(int(rentdate[0:4]), int(rentdate[4:6]), int(rentdate[6:8])).weekday()] is 'Mon':
        output.append(rent['RENT_STATION'])
        station[int(rent['RENT_STATION'])]['count'] += 1
    elif getDay[datetime.date(int(rentdate[0:4]), int(rentdate[4:6]), int(rentdate[6:8])).weekday()] is 'Tues':
        output.append(rent['RENT_STATION'])
        station[int(rent['RENT_STATION'])]['count'] += 1
    elif getDay[datetime.date(int(rentdate[0:4]), int(rentdate[4:6]), int(rentdate[6:8])).weekday()] is 'Wednes':
        output.append(rent['RENT_STATION'])
        station[int(rent['RENT_STATION'])]['count'] += 1
    elif getDay[datetime.date(int(rentdate[0:4]), int(rentdate[4:6]), int(rentdate[6:8])).weekday()] is 'Thurs':
        output.append(rent['RENT_STATION'])
        station[int(rent['RENT_STATION'])]['count'] += 1
    elif getDay[datetime.date(int(rentdate[0:4]), int(rentdate[4:6]), int(rentdate[6:8])).weekday()] is 'Fri':
        output.append(rent['RENT_STATION'])
        station[int(rent['RENT_STATION'])]['count'] += 1
